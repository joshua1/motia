...
export const generateTypesFromSteps = (steps: Step[], printer: Printer): HandlersMap => {
	...
	for (const step of steps) {
		if (isEventStep(step)) {
			...
			for (const topic of step.config.subscribes) {
				...
				try {
					const input = schemaToJsonSchema(step.config.input as any) as JsonSchema
	...
	for (const step of steps) {
		...
		if (isEventStep(step)) {
			const input = step.config.input ? generateTypeFromSchema(schemaToJsonSchema(step.config.input) as JsonSchema) : 'never'
			handlers[step.config.name] = { type: 'EventHandler', generics: [input, emits] }
		} else if (isApiStep(step)) {
			const input = step.config.bodySchema
				? generateTypeFromSchema(schemaToJsonSchema(step.config.bodySchema) as JsonSchema)
				: 'Record<string, unknown>'
			const result = step.config.responseSchema
				? generateTypesFromResponse(
					Object.fromEntries(
						Object.entries(step.config.responseSchema).map(([status, schema]) => [
							status,
							schemaToJsonSchema(schema) as JsonSchema
						])
...
export const generateTypesFromStreams = (streams: Record<string, Stream>): StreamsMap => {
	return Object.entries(streams).reduce((acc, [key, stream]) => {
		if (!stream.hidden) {
			acc[key] = generateTypeFromSchema(schemaToJsonSchema(stream.config.schema as any))
		}
		return acc
	}, {} as StreamsMap)
}
